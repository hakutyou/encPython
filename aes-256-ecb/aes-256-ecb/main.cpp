...
int fgets_force(unsigned char *buf, unsigned size, FILE *fp, int do_memset) {
	unsigned count = 0;
	if (do_memset) {
		memset(buf, 0, sizeof(char) * (size + 1));
	}
	while (!feof(fp) && (size > count)) {
		if (buf[count] == 0xff) {
			break;
		}
		buf[count++] = fgetc(fp);
...
void fputs_force(unsigned char *buf, unsigned size, FILE *fp) {
	unsigned count = 0;
	while (size > count) {
		fputc(buf[count++], fp);
	}
...
int main(int argc, char *argv[]) {
	...
	FILE *fp, *fp_w;
	fopen_s(&fp, argv[1], "r");
	fopen_s(&fp_w, argv[2], "wb");
	unsigned char *key = (unsigned char *)argv[3];
	// 每次 16 个单位加密
	unsigned char cipher[16 * CIPHER_UNIT + 1];
	unsigned char plain[16 * CIPHER_UNIT + 1];
	cipher[16 * CIPHER_UNIT] = plain[16 * CIPHER_UNIT] = 0;
	// 加密
	if (!fp) {
		printf("file open failed!\n");
		return -1;
	}
	fputs_force((unsigned char *)"*", 1, fp_w);
	while (fgets_force(plain, 16 * CIPHER_UNIT, fp, 1)) {
		aes_encrypt_ecb(key, 16, plain, cipher, CIPHER_UNIT);
		fputs_force(cipher, 16 * CIPHER_UNIT, fp_w);
	}
	fclose(fp);
	fclose(fp_w);
#else
	...
	FILE *fp_r;
	fopen_s(&fp_r, argv[1], "rb");
	unsigned char *key = (unsigned char *)argv[2];
	printf("Passwd=%s\n", key);
	// 每次 16 个单位解密
	unsigned char cipher_r[16 * CIPHER_UNIT + 1];
	unsigned char plain_r[16 * CIPHER_UNIT + 1];
	cipher_r[16 * CIPHER_UNIT] = plain_r[16 * CIPHER_UNIT] = 0;
	// 解密
	if (!fp_r) {
		printf("file open failed!\n");
		return -1;
	...
	int cc = 0;
	int is_feof = 0;
	while (!is_feof && (cc=fgets_force(cipher_r, 16 * CIPHER_UNIT, fp_r, 1))) {
		// printf("%d %d\n", cipher_r[0], cipher_r[1]);
		memset(plain_r, 0, 17);
		if (aes_decrypt_ecb(key, 16, cipher_r, plain_r, CIPHER_UNIT) == 0x0) {
			for (int i = 0; i < 16 * CIPHER_UNIT; i++) {
				if (plain_r[i] == 0xff) {
				 	plain_r[i] = 0;
					is_feof = 1;
				}
